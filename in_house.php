                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form class="from-group" action="func.php" method="post">
                                <label><h5>Your in-house patients</h5></label><br>
                                <?php $count=get_count_in_house();
                                echo "<h5 style='color:#08827F'>$count</h5>";?>
                                <hr>
                                <label>Select patient</label>
                                <select class="form-control" name="pname">
                                    <?php $result=dropdown_for_in_house();?>
                                    <?php while ($row=mysqli_fetch_array($result)):;?>
                                    <option value="<?php echo $row['pname'];?>"><?php echo $row['pname'];?></option>
                                    <?php endwhile; ?>
                                </select><br>
                                <input type="submit" class="btn btn-primary" name="in_house_1" value="Next">
                            </form>
                        </div>
                    </div>
                </div>
